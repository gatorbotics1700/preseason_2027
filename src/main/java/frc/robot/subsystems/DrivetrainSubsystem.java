...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public static double busUtil =
      CANivore.getStatus().BusUtilization * 100; // bus utilization percentage
  ...
  public DrivetrainSubsystem() {
    ...
    frontLeftModule =
        new MkSwerveModuleBuilder()
            ...
            .withSteerEncoderPort(
                Constants.FRONT_LEFT_MODULE_STEER_ENCODER, Constants.CANIVORE_BUS_NAME)
    ...
    frontRightModule =
        new MkSwerveModuleBuilder()
            ...
            .withSteerEncoderPort(
                Constants.FRONT_RIGHT_MODULE_STEER_ENCODER, Constants.CANIVORE_BUS_NAME)
    ...
    backLeftModule =
        new MkSwerveModuleBuilder()
            ...
            .withSteerEncoderPort(
                Constants.BACK_LEFT_MODULE_STEER_ENCODER, Constants.CANIVORE_BUS_NAME)
    ...
    backRightModule =
        new MkSwerveModuleBuilder()
            ...
            .withSteerEncoderPort(
                Constants.BACK_RIGHT_MODULE_STEER_ENCODER, Constants.CANIVORE_BUS_NAME)
    ...
    DCMotor krakenMotor =
        DCMotor.getKrakenX60(1); // TODO: check if we change this number to 1 or keep
    ...
    AutoBuilder.configure(
        ...
        new PPHolonomicDriveController(new PIDConstants(5, 0, 0.05), new PIDConstants(10, 0, 0.01)),
  ...
  public void setStates(SwerveModuleState[] targetStates) {
    ...
    double fl_voltage =
        (targetStates[0].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND) * MAX_VOLTAGE;
    double fr_voltage =
        (targetStates[1].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND) * MAX_VOLTAGE;
    double bl_voltage =
        (targetStates[2].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND) * MAX_VOLTAGE;
    double br_voltage =
        (targetStates[3].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND) * MAX_VOLTAGE;
  ...
  public void drive(ChassisSpeeds chassisSpeeds) {
    ...
    ChassisSpeeds targetSpeeds =
        ChassisSpeeds.discretize(chassisSpeeds, Constants.LOOPTIME_SECONDS);
  ...
  public void periodic() {
    ...
    LimelightHelpers.SetRobotOrientation(
        "limelight", odometry.getEstimatedPosition().getRotation().getDegrees(), 0, 0, 0, 0, 0);
    ...
    if (mt2 != null) {
      ...
      if (!doRejectUpdate) {
        // this line sets the amount the pigeon odometry can be off by in each axis before vision
        // odometry takes over. we trust the pigeon for heading, hence the reaaaaally big number
