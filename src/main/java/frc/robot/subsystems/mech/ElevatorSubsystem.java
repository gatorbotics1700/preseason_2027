...
public class ElevatorSubsystem extends SubsystemBase {
  private final TalonFX motor = new TalonFX(0, TunerConstants.mechCANBus); // placeholder value
  ...
  public ElevatorSubsystem() {
    ...
    motor.setNeutralMode(
        NeutralModeValue
            .Brake); // indicates that motor will actively stop rather than spin freely to a stop
  }
  ...
  public void setTargetPose(double rotations) {
  ...
  public boolean atTargetPose(double goal, double error) {
    if (Math.abs(goal - motor.getPosition().getValueAsDouble()) <= error) {
      return true;
    }
    return false;
  }
