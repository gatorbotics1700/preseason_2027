 ...
 * detected target pitch/yaw from PhotonVision, and the theoretically correct outputs. The tests
 * will verify that Vision.java produces those expected values.
 *
...
class VisionMathTest {
  ...
  private static final boolean TEMPLATE_IS_FILLED_IN = false;

  /** Config file to load (contains robot-to-camera transforms). e.g. "configFiles/config_sting.properties" */
  ...
  private static final double TEMPLATE_ROBOT_POSE_X_METERS = 0.0;

  private static final double TEMPLATE_ROBOT_POSE_Y_METERS = 0.0;
  ...
  private static final double TEMPLATE_TARGET_PITCH_DEGREES = 0.0;
   ...
   * (camera_height_cm - 15) / cos(|camera_pitch_rad| + toRadians(target_pitch_deg))
   * Units: centimeters.
   */
  private static final double TEMPLATE_EXPECTED_DISTANCE_CM = 0.0;

  /** Expected fuel pose (field coordinates) from getFuelPose. Units: meters, degrees. */
  private static final double TEMPLATE_EXPECTED_FUEL_POSE_X_METERS = 0.0;
  ...
  private static final double TEMPLATE_ANGULAR_TOLERANCE_DEGREES = 2.0;
  ...
  void testGetFuelPose() {
    ...
    Pose2d actual = getFuelPoseFromMockedVision(robotPose, TEMPLATE_TARGET_PITCH_DEGREES, TEMPLATE_TARGET_YAW_DEGREES);

    assertEquals(
        expectedFuelPose.getX(),
        actual.getX(),
        TEMPLATE_POSE_TOLERANCE_METERS,
        "Fuel pose X");
    assertEquals(
        expectedFuelPose.getY(),
        actual.getY(),
        TEMPLATE_POSE_TOLERANCE_METERS,
        "Fuel pose Y");
    assertEquals(
